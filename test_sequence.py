#!/usr/bin/env python3
"""
Test the 90°-x → delay → 90°-y → acquire sequence
"""
...
def test_sequence():
    """Test the classic sequence: 90°-x → 1/(2J) delay → 90°-y → acquire"""
    print("Testing 90°-x → delay → 90°-y → acquire sequence")
    ...
    # Step 3: 90°-y pulse on A
    nmr.pulse(90, 'y', 'A')
    Mx_A = nmr.gamma_A * np.trace(nmr.rho @ nmr.Ax)
    My_A = nmr.gamma_A * np.trace(nmr.rho @ nmr.Ay)
    Mx_antiphase = 2 * nmr.gamma_A * np.trace(nmr.rho @ (nmr.Ax @ nmr.Kz))
    My_antiphase = 2 * nmr.gamma_A * np.trace(nmr.rho @ (nmr.Ay @ nmr.Kz))
    print(f"After 90°-y on A: Mx_A={Mx_A:.6f}, My_A={My_A:.6f}")
    print(f"Antiphase terms: Mx_antiphase={Mx_antiphase:.6f}, My_antiphase={My_antiphase:.6f}")
    ...
    plt.tight_layout()
    plt.savefig('test_sequence_results.png', dpi=150, bbox_inches='tight')
    print("Plot saved as 'test_sequence_results.png'")
